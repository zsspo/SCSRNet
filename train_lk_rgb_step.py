...
# TRAIN EPOCH
def train(epoch, update_freq=None):
    train_loss = 0.0
    model.train()
    ...
    optimizer.zero_grad()
    for i, data in enumerate(metric_logger.log_every(train_loader, print_freq, header)):
        # Reading data
...
def test(data_loader, model, criterion):
    ...
    for i, data in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        if config['train_dataset'] =='road_train_dataset' or config['train_dataset'] =="dx_dataset" :
            pred_dic.update({image_dict["hrhsi"][0].split("/")[-1][:-4]+"_pred": torch.squeeze(outputs).permute(1,2,0).cpu().numpy()})
        elif config['train_dataset'] == "mdasn_dataset":
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', default='config_lk/config_rgb_lk_all_p.json', type=str, help='Path to the config file')
    ...
    num_training_steps_per_epoch = len(train_loader) // total_batch_size  #32/8
    print("LR = %.8f" % args.lr)
    ...
    # Optimizer using Cosine LR scheduler
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        train_stats=train(epoch, update_freq=args.update_freq)
        ...
        if epoch%config["trainer"]["test_freq"] == 0:
            ...
            if best_psnr < metrics['psnr']:
                ...
                savemat(PATH + "/" + "val_prediction.mat", pred_dic)
    ...
    image_dict, test_pred_dic, test_metrics, test_stats = test(test_loader, best_model, criterion)  
    ...
    savemat(PATH + "/" + "final_test_prediction.mat", test_pred_dic)
