...
def test(epoch):
    ...
    with torch.no_grad():
        for i, data in enumerate(test_loader, 0):
            ...
            outputs                 = torch.round(outputs*config[config["train_dataset"]]["max_value"]).contiguous()
            ...
            reference = torch.round(reference.detach()*config[config["train_dataset"]]["max_value"]).contiguous()
...
if __name__ == "__main__":
    ...
    __dataset__ = {"pavia_dataset":    pavia_dataset, "botswana_dataset": botswana_dataset,
                   "chikusei_dataset": chikusei_dataset, "botswana4_dataset": botswana4_dataset, "dx_dataset": dx_dataset,"mdasn_dataset": mdasn_dataset
        }

    # PARSE THE ARGS
    parser = argparse.ArgumentParser(description='PyTorch Training')
    parser.add_argument(
        '-c', '--config', default='config_lk/config_rgb.json', type=str, help='Path to the config file'
        )
    parser.add_argument(
