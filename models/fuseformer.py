...
class Fuseformer(nn.Module):
    ...
    def forward(self, HSI, MSI):
        ################LR-HSI###################
        UP_LRHSI = F.interpolate(HSI,scale_factor=self.factor, mode='bicubic') ### (b N h w)
...
class Transformer_E(nn.Module):
    def __init__(self, dim, depth=2, heads=1, dim_head=2, mlp_dim=16, sp_sz=64*64, num_channels = 16,dropout=0.):
        super().__init__()
