...
class PSRTnet(nn.Module):
    def __init__(self, config):
        ...
        self.img_size =  config[config["train_dataset"]]["HR_size"]
        self.in_channels = config[config["train_dataset"]]["spectral_bands"]
        self.embed = 32
        ...
        self.w = Block(out_num=2, inside_num=3, img_size=self.img_size, in_chans=self.in_channels+self.msi_channels, embed_dim=self.embed, head=8,
                       win_size=8)
    ...
    def forward(self, lms, rgb):
        ...
        '''
        self.rgb = rgb
        self.lms = F.interpolate(lms,scale_factor=self.factor, mode='bicubic') ### (b N h w)
