...
class HSI_Fusion(nn.Module):
    def __init__(self, config):
        ...
        self.Ch = config[config['train_dataset']]['spectral_bands']
        self.s  = stages
        self.sf = config[config['train_dataset']]['factor']
        self.kernel_size = [7, 7, 7]

        ## The modules for learning the measurement matrix R and R^T
        self.RT = nn.Sequential(nn.Conv2d(3, self.Ch, kernel_size=3, stride=1, padding=1), nn.LeakyReLU())
        self.R  = nn.Sequential(nn.Conv2d(self.Ch, 3, kernel_size=3, stride=1, padding=1), nn.LeakyReLU())

        ## The modules for learning the measurement matrix B and B^T
        if self.sf == 8:
            self.BT = nn.Sequential(nn.ConvTranspose2d(self.Ch, self.Ch, kernel_size=12, stride=8, padding=2), nn.LeakyReLU())
            self.B  = nn.Sequential(nn.Conv2d(self.Ch, self.Ch, kernel_size=12, stride=8, padding=2), nn.LeakyReLU())
        elif self.sf == 16:
            self.BT = nn.Sequential(nn.ConvTranspose2d(self.Ch, self.Ch, kernel_size=6, stride=4, padding=1),
                                    nn.LeakyReLU(),
                                    nn.ConvTranspose2d(self.Ch, self.Ch, kernel_size=6, stride=4, padding=1), nn.LeakyReLU())
            self.B = nn.Sequential(nn.Conv2d(self.Ch, self.Ch, kernel_size=6, stride=4, padding=1),
                                   nn.LeakyReLU(),
                                   nn.Conv2d(self.Ch, self.Ch, kernel_size=6, stride=4, padding=1), nn.LeakyReLU())
        ...
        self.conv  = nn.Conv2d(self.Ch+3, 64, kernel_size=3, stride=1, padding=1)
