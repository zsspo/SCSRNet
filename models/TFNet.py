...
class ResTFNet(nn.Module):
    def __init__(self, config):
        ...
        self.lr_conv1 = nn.Sequential(
                  nn.Conv2d(self.n_bands, 32, kernel_size=3, stride=1, padding=1),
                  nn.PReLU(),
                )
        ...
        self.hr_conv1 = nn.Sequential(
                  nn.Conv2d(self.n_select_bands, 32, kernel_size=3, stride=1, padding=1),
                  nn.PReLU(),
                )
        ...
        self.recons_conv6 = nn.Sequential(
                  nn.Conv2d(64, self.n_bands, kernel_size=3, stride=1, padding=1),
                  nn.PReLU(),
                )
