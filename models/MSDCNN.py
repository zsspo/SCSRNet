...
class MSDCNN(nn.Module):
    def __init__(self, config):
        ...
        self.shallow_conv = nn.Sequential(
                  nn.Conv2d(self.n_bands+self.n_select_bands, 64, kernel_size=9, stride=1, padding=4),
                  nn.PReLU(),
                  nn.Conv2d(64, 32, kernel_size=1, stride=1, padding=0),
                  nn.PReLU(),
                  nn.Conv2d(32, self.n_bands, kernel_size=5, stride=1, padding=2),
                  nn.PReLU(),
                )
        self.deep_conv_a = nn.Sequential(
                  nn.Conv2d(self.n_bands+self.n_select_bands, 60, kernel_size=7, stride=1, padding=3),
                  nn.PReLU(),
                )
        ...
        self.deep_conv_e = nn.Sequential(
                  nn.Conv2d(30, self.n_bands, kernel_size=5, stride=1, padding=2),
                  nn.PReLU(),
                )
